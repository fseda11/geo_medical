        ...
        for _, muni in top_munis.iterrows():
            ...
            if coords:
                folium.PolyLine(
                    ...
                    color="#1565C0",
                    opacity=0.55,
                    tooltip=f"Rota → {muni.get('nome','')} ({muni.get('road_km','')} km)",
                ).add_to(route_layer)

    route_layer.add_to(m)

    # ── Municípios alcançáveis ─────────────────────────────────────────────────
    muni_layer = folium.FeatureGroup(name="🏙️ Municípios na rota", show=True)

    if not municipalities.empty:
        for _, muni in municipalities.iterrows():
            road_km = float(muni.get("road_km") or 0)
            color = _distance_color(road_km, max_km)

            folium.CircleMarker(
                location=[muni["latitude"], muni["longitude"]],
                radius=6,
                color=color,
                fill=True,
                fill_color=color,
                ...
                weight=1.5,
                tooltip=(
                    f"<b>{muni.get('nome','')}</b> — {muni.get('uf','')}<br>"
                    f"📍 {road_km:.0f} km por estrada ({muni.get('duration_text','')})"
                ),
            ).add_to(muni_layer)
