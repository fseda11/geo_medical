...
with st.sidebar:
    ...
    if st.session_state.result_map is not None and st.session_state.show_results:
...
if st.session_state.result_map is not None:
    ...
    with _fc1:
        _show_alto  = st.checkbox("🟢 Alto (≥ 60)",   value=True, key="f_alto")
    with _fc2:
        _show_medio = st.checkbox("🟠 Médio (40–59)", value=True, key="f_medio")
    with _fc3:
        _show_baixo = st.checkbox("⚫ Baixo (< 40)",  value=True, key="f_baixo")

    _est_map = establishments.copy()
    if not (_show_alto and _show_medio and _show_baixo):
        _masks = []
        if _show_alto:  _masks.append(_est_map["score_potencial"] >= 60)
        if _show_medio: _masks.append((_est_map["score_potencial"] >= 40) & (_est_map["score_potencial"] < 60))
        if _show_baixo: _masks.append(_est_map["score_potencial"] < 40)
        import functools, operator as _op
        _est_map = _est_map[functools.reduce(_op.or_, _masks)] if _masks else _est_map.iloc[0:0]
    ...
    st_folium(_fmap_filtered, use_container_width=True, height=640, returned_objects=[])
